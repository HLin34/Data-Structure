...
int main()
{
    BST<int, char> tree({50, 5, 30, 40, 80, 35, 2, 20, 15, 60, 70, 8, 10});
    pair<int, char>* data = tree.Get(30);
    cout << data->first << endl;
    cout << (int)data->second << endl;
    ...
    for (int i = 1; i < 14; i++)
    {
        pair<int, char>* target = tree.RankGet(i);
        cout << target->first << " " << (int)target->second << endl;
    }
    ...
    tree.Split(4, tree_a, Data, tree_b);
    tree_a.InorderPrint();
